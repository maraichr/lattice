...
start_databases() {
  ...
  log "starting postgres + neo4j via docker compose..."
  if ! docker compose up -d postgres neo4j; then
    log "WARN: could not start database containers (image pull blocked?)."
    log "      Allowlist the container registry in the environment network policy:"
    log "      *.docker.io, auth.docker.io, production.cloudfront.docker.com, *.cloudflare.docker.com"
    return 1
  fi
